describe("TC-UPDATE-ANNOUNCEMENT-1: normal - update title and description", () => {
  it('Add an announcement "(PBI7-1)ประกาศ (ร่าง) ตารางสอบปลายภาค 2/65"', () => {
    ...
    cy.get(".ann-description").type(
      "หลักสูตรขอแจ้งตารางสอบปลายภาค (ร่าง) 2/65 โดยนักศึกษาที่สอบซ้อนในวันเดียวกัน ขอให้แจ้งกลับมาที่พี่ตุ๊ก ภายในวันที่ 8 พ.ค. 66"
    );
    cy.get(".ann-publish-date").type("2023-05-03");
    cy.get(".ann-publish-time").type("06:00");
    cy.get(".ann-button")
      .contains(/submit|save|update|add|edit/g, { matchCase: false })
      .click();
    cy.wait(200);
  });
  ...
  it('should contain the announcement "(PBI7-1)ประกาศ (ร่าง) ตารางสอบปลายภาค 2/65"', () => {
    ...
    cy.wait(200);
    cy.get(".ann-item")
      .should("contain.text", "(PBI7-1)ประกาศ (ร่าง) ตารางสอบปลายภาค 2/65")
      .first();
  });
  ...
  it("should have pre-filled data in form", () => {
    ...
    cy.wait(200);
    cy.get(".ann-title").should(
      "have.value",
      "(PBI7-1)ประกาศ (ร่าง) ตารางสอบปลายภาค 2/65"
    );
    cy.get(".ann-category").contains("หางาน");
    cy.get(".ann-description").should(
      "have.value",
      "หลักสูตรขอแจ้งตารางสอบปลายภาค (ร่าง) 2/65 โดยนักศึกษาที่สอบซ้อนในวันเดียวกัน ขอให้แจ้งกลับมาที่พี่ตุ๊ก ภายในวันที่ 8 พ.ค. 66"
    );
  ...
  it("should update title and description and should enable submit button", () => {
    ...
    cy.get(".ann-title")
      .clear()
      .type("(PBI7-1)ประกาศ (ร่าง) ตารางสอบปลายภาค 2/65 ครั้งที่ 1");
    cy.get(".ann-description")
      .clear()
      .type(
        "หลักสูตรขอแจ้งตารางสอบปลายภาค (ร่าง) 2/65 ครั้งที่ 1 โดยนักศึกษาที่สอบซ้อนในวันเดียวกัน ขอให้แจ้งกลับมาที่พี่ตุ๊ก ภายในวันที่ 8 พ.ค. 66"
      );
    cy.get(".ann-button")
      .contains(/submit|save|update|add|edit/g, { matchCase: false })
      .as("ann-submit");
  ...
  it("should show the updated data in both listing and detail page .", () => {
    ...
    cy.get(".ann-item")
      .should(
        "contain.text",
        "(PBI7-1)ประกาศ (ร่าง) ตารางสอบปลายภาค 2/65 ครั้งที่ 1"
      )
      .first()
      .as("ann-1");
    cy.get("@ann-1")
    ...
    cy.wait(200);
    cy.get(".ann-title").contains(
      "(PBI7-1)ประกาศ (ร่าง) ตารางสอบปลายภาค 2/65 ครั้งที่ 1"
    );
    cy.get(".ann-description").contains(
      "หลักสูตรขอแจ้งตารางสอบปลายภาค (ร่าง) 2/65 ครั้งที่ 1 โดยนักศึกษาที่สอบซ้อนในวันเดียวกัน ขอให้แจ้งกลับมาที่พี่ตุ๊ก ภายในวันที่ 8 พ.ค. 66"
    );
    cy.get(".ann-category").contains("หางาน");
